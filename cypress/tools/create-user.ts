...
async function createUser() {
  ...
  await manager.query(
    `INSERT INTO patient(firebaseKey, firstName, lastName, isEmailVerified) VALUES('${testData.ID}', '${testData.FIRST_NAME}', '${testData.LAST_NAME}', ${testData.IS_EMAIL_VERIFIED}) ON DUPLICATE KEY UPDATE isEmailVerified = ${testData.IS_EMAIL_VERIFIED};`,
  )
  const [patient] = await manager.query(
    `SELECT * FROM patient WHERE patient.firebaseKey='${testData.ID}' LIMIT 1`,
  )

  await manager.query(
    `INSERT INTO patientAuth(patientId, authUserId, email, phoneNumber) VALUES('${patient.idPatient}', '${authUser.uid}', '${testData.EMAIL}', ${testData.USER_PHONE_NUMBER}) ON DUPLICATE KEY UPDATE patientAuth.authUserId='${authUser.uid}';`,
  )
}
